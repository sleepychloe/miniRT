...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   raytracing_cone_texture_1_bonus.c                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
				...
				t_hit *hit, int co_i, double *theta_height)
{
	...
	uv[0] += 0.05;
	u_int = (int)(uv[0] * data->obj[co_i]->texture_width);
	v_int = (int)(uv[1] * data->obj[co_i]->texture_height);
	u_int /= 4;
	v_int /= 4;
...
void	cone_texture(t_data *data, t_hit *hit, int co_i)
{
			...
			&& data->obj[co_i]->xyz_vec.y != 0))
	{
		if (fabs(data->obj[co_i]->xyz_vec.z) == 0)
			...
			cone_texture_norminette(data, hit, co_i);
		else
			cone_texture_zx(data, hit, co_i);
	}
	else
