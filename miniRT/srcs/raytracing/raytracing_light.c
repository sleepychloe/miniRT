...
/*   Updated: 2022/11/14 05:32:34 by yhwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_light_hit_obj(t_scene *scene, int cnt, t_ray r, double lim)
{
	...
	while (i < cnt)
	{
		if (interfere_sp(scene, r, cnt - 1, lim))
			return (1);
		i++;
	}
	return (0);
...
int	calculate_light(t_scene *scene, t_hit *hit, double *s, t_rgb3 *tc)
{
	...
	to = vec3_sub_vec3(hit->hit_point, scene->light->xyz_pos);
	radius_squared = vec3_dot_vec3(to, to);
	td = sqrt(radius_squared);
	to = vec3_unit(to);
	*tc = rgb3(scene->light->rgb.r * scene->light->brightness,
			scene->light->rgb.g * scene->light->brightness,
			scene->light->rgb.b * scene->light->brightness);
	...
	*s = vec3_dot_vec3(hit->normal_vec, vec3_mul_rn(to, -1));
	if (*s < 0)
		*s = 0;
	return (check_light_hit_obj(scene, scene->n_sphere, r, td));
}
...
t_rgb3	apply_light(t_scene *scene, t_hit *hit)
{
	...
	light = calculate_light(scene, hit, &s, &tc);
